...
def segment_plane(
    pcd, distance_threshold=0.01, ransac_n=3, num_iterations=1000, probability=1.0
) -> (Plane, np.array):
    ...
    plane = Plane(plane_model[0], plane_model[1], plane_model[2], plane_model[3])
...
def restricted_uv2xyz(uv: np.array, camK: np.array, plane: Plane) -> np.array:
    ...
    z = -(
        plane.d
        / (plane.a * (uv[:, 0] - cx) / fx + plane.b * (uv[:, 1] - cy) / fy + plane.c)
    )
    x = (uv[:, 0] - cx) / fx * z
    y = (uv[:, 1] - cy) / fy * z
